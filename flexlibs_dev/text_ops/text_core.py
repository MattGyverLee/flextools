...
class TextCoreOperations:
    ...
    def text_create(self, name: str, genre: Optional[str] = None) -> IText:
        ...
        if self.text_exists(name):
            raise ValueError(f"Text with name '{name}' already exists")
    ...
    def text_set_genre(self, text_or_hvo, genre: str) -> None:
        ...
        Raises:
            ValueError: If the text or genre does not exist
    ...
    def _find_genre(self, genre_name: str):
        """
        Helper method to find a genre by name.

        Args:
            genre_name (str): The name of the genre to find

        Returns:
            Genre object or None if not found
        """
        # TODO: Integrate with FLEx API
        # genre_list = self.project.LangProject.GenreListOA
        # for genre in genre_list.PossibilitiesOS:
        #     if genre.Name.BestAnalysisAlternative.Text == genre_name:
        #         return genre
        # return None
